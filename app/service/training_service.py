...
_RULE_ADJUST_SYS = (
    ...
    "你的任务是分析原因并给出调整后的规则配置，使规则能准确命中该命中的物料、避免误命中不该命中的物料。\n\n"
    "调整策略:\n"
    "- 漏判 → 放宽覆盖:补充关键词的同义词/近义词/常见变体,在 condition 中把漏判物料里的典型表述纳入违规描述,"
    "适当降低 guidance 里的判定门槛。\n"
    "- 多判 → 收紧边界:在 guidance 里加入反例(什么不算违规、什么情况应放行),细化 condition 描述把边界写清楚。\n"
    "- 注意:宁可保留少量多判(交人工复核),也必须消除漏判。\n\n"
    "只返回一个合法 JSON 对象,不要 markdown、不要多余解释,字段:\n"
    "analysis(中文,简要分析漏判/多判的原因,≤200字),\n"
    "keywords(调整后的关键词数组,保留原有有效的、移除不相关的、补充漏判物料里出现的关键词同义/近义/变体表达),\n"
    "condition(调整后的自然语言条件描述,把违规边界写清楚),\n"
    "guidance(调整后的尺度说明,必须包含:什么算违规、什么不算违规、好例/坏例),"
    "保留下之前 guidance 里仍然有效的部分,\n"
    "match_level(保持原值,或根据需要建议改为 literal/metaphor/regex;"
    "漏判严重且语义难以区分时可从 literal 提升到 metaphor)。"
)
