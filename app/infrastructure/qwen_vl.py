...
class QwenVLVisionDescriber:
    ...
    _PROMPT = (
        ...
        "【防幻觉】抄录文字必须逐字确认,宁缺毋滥;"
        "看不清的文字标注为「文字模糊不可辨」,严禁猜测内容;"
        "平台名称（如QQ音乐、微信、网易云等）只有清晰可辨时才写出,不得从图标或UI风格推测。"
        "只客观描述实际看到的内容,不要额外做风险总结或抽象归类。"
        "【严禁】不要输出否定句（如「未见」「没有」「无」「不涉及」「未出现」等）;"
        "严禁逐条罗列画面中「不存在」的内容,这会导致审核系统误判;"
        "只写你看到的,不写你没看到的。"
    )
    ...
    def describe_image(self, url: str, hints: str = "") -> str:
        ...
        if hints:
            prompt += (
                "\n\n【审核特别关注项】请特别留意以下内容,如果确实出现在画面中,在描述时明确写出"
                "（包括具体是什么、在画面的什么位置）:"
                "\n" + hints
                + "\n\n【关键纪律】以上关注项只用于指引你的注意力——"
                "只描述你实际看到的,严禁逐条对照输出「未出现」「无」「没有」等否定罗列;"
                "关注项不在画面中的,保持沉默,不要提及。"
            )
