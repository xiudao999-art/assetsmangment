...
class QwenVLVisionDescriber:
    ...
    _PROMPT = (
        ...
        "只输出纯文本,不要 Markdown,不要标题,不要分隔线,不要项目符号,不要编号。"
        "按自然语序连续描述主体、场景、动作、画面中的文字、服饰、姿态、道具、环境和风格,"
        "以及任何可能涉及违规的风险点(如暴力、色情、政治敏感、违禁品等)。"
        ""
        "描述的同时逐字抄录画面中出现的所有文字,包括小字、水印、免责声明、金额数字、"
        "提现门槛、平台名称、下载引导语等,即使文字很小或位于边缘也必须抄录。"
        "每处文字必须同时注明其在画面中的具体位置,"
        "使用「画面顶部」「下方居中」「右上角」等方位词,"
        "例如「画面下方居中位置有三行白色小字:第一行'满0.3元能提现'……」。"
        ""
        "抄录必须逐字确认,看不清的文字标注为「文字模糊不可辨」,不得猜测内容;"
        "平台名称只有清晰可辨时才写出,不得从图标或UI风格推测。"
        "如果画面中确实没有任何文字,直接描述画面内容即可。"
        ""
        "描述末尾单独一行注明免责文字位置,格式:"
        "「免责文字位置:下方居中」或「免责文字位置:顶部」"
        "或「免责文字位置:右上角」或「免责文字位置:右下角」"
        "或「免责文字位置:左下角」或「免责文字位置:未出现」。"
        "【重要】免责文字位置必须精确判断:文字在画面底部且水平居中才算「下方居中」;"
        "文字靠近底部但偏左→「左下角」,偏右→「右下角」。不要将所有底部文字笼统标注为下方居中。"
        ""
        "只写你实际看到的,不写你没看到的——这是硬性要求,不是建议。"
        "严禁以任何否定句式罗列不存在的内容,"
        "包括但不限于「画面中未出现…」「无…」「未见…」「不存在…」等开头句式。"
        "描述完实际画面即可结束,不得在末尾追加一段否定声明清单。"
    )
    ...
    def describe_image(self, url: str, hints: str = "") -> str:
        ...
        if hints:
            prompt += (
                "\n\n【审核关注项】请特别留意以下内容,出现在画面中时明确写出具体位置:"
                "\n" + hints
                + "\n以上是关注指引,不是逐条对照清单。只写实际出现的,不出现的保持沉默。"
                + "\n严禁以「未出现XX」「无XX」「未见XX」等否定句式逐条声明。"
            )
