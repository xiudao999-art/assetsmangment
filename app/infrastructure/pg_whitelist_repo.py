"""内容安全白名单仓储 —— PostgreSQL 真源实现(实现 domain.ports.WhitelistRepo)。
...
class PgWhitelistRepo:
    def __init__(self, dsn: str, table: str = "content_whitelist", idgen=None) -> None:
    ...
    def _init_schema(self) -> None:
        ...
        with self._conn() as c:
            ...
            c.execute(f"COMMENT ON TABLE {t} IS '内容安全白名单。命中这些词时即便阿里云判违规也放行。'")
            c.execute(f"COMMENT ON COLUMN {t}.id IS '雪花算法 BIGINT 主键'")
            c.execute(f"COMMENT ON COLUMN {t}.word IS '白名单词条'")
